...
\includegraphics[width=4cm,height=3cm]{isomorphic} \\ \centering H 
  ...
  \begin{Def}
    ...
    其中$x_i=v_{i-1}v_i,i=1,2,\ldots,n$。$n$称为通道的长。这样的通道常称为$v_0-v_n$通道，并简记为$v_0v_1v_2\ldots   v_n$。当$v_0=v_n$时，则称此通道为闭通道。
  \end{Def}
  \begin{Def}
   如果图中一条通道上的各边互不相同，则称此通道为图的迹。如果一条闭通道上的各边互不相同，则此闭通道称为闭迹。
  \end{Def}
  \begin{Def}
