...
\begin{proof}[证法二]设$G$为一个没有三角形，包含$p$个顶点且边数$q\geq [\frac{p^2}{4}]$的图。设$V$为$G$的顶点集合，$v$为$G$中度最大的顶点，$V_1$为所有与$v$邻接的顶点构成的集合，$V_2=V\setminus V_1$。以下证明$V_1$中的顶点互相不邻接，$V_2$中的顶点互相不邻接，$|V_1|$和$|V_2|$最多相差$1$，从而完成定理的证明。
