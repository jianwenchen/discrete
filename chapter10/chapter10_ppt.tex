...
\begin{frame}
  ...
  \begin{Def}
    ...
    其中$x_i = (v_{i-1},v_i)$， $i=1,2,\cdots, n$。$n$称为该有向通道的长。 这样的有向通道常称为$v_0-v_n$有向通道，并简记为$v_0v_1v_2\ldots v_n$。如果有向通道的长大于等于$1$且$v_0=v_n$,则称此有向通道为\alert{闭有向通道}。
  \end{Def}
  \centering
  \begin{tikzpicture}[auto,
    specification/.style ={circle, draw, thick}]
...
\begin{frame}
  \frametitle{2. 有向路和有向圈}
  \begin{Def}
如果一条有向迹上的各顶点互不相同，则称此有向迹为\alert{有向路}。如果闭有向迹上除终点外各顶点互不相同，则称此闭有向迹为\alert{有向圈}，或\alert{有向回路}。
  \end{Def}
  \centering
  \begin{tikzpicture}[auto,
    specification/.style ={circle, draw, thick}]
